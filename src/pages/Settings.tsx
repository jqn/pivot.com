...
function SettingCard({ title, label, description, enabled, onToggle, children }: SettingCardProps) {
  return (
    <div
      style={{
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <input
              ...
              type="number"
              value={rsiThreshold}
              onChange={(e) => setRsiThreshold(Math.max(1, Math.min(100, Number(e.target.value))))}
              min={1}
              max={100}
              style={{
                padding: '7px 12px',
      ...
      >
        ...
        <div
          style={{
            fontFamily: "'DM Mono', monospace",
            fontSize: '28px',
            color: [rsiEnabled, smaEnabled, macdEnabled].filter(Boolean).length > 0
              ? 'var(--signal)'
              : 'var(--warn)',
            fontWeight: '500',
          }}
